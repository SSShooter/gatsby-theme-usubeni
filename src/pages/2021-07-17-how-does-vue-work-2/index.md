...
想想嘛，根据前一篇解析的 Vue 响应式原理，响应式数据在赋值时运行 setter，触发视图更新，但是就像上面例子中连续赋值的场景可以说十分常见了，在 React 倒是可以写在一个 setState 里，但是 Vue 没有这种方法。

总不能每次更新数据都立即给你刷新一次页面，如果每次赋值都更新，那不是会卡得不行吗？

所以解决这个问题的关键就是先把需要运行的更新函数都存入队列，再异步运行队列。而且，在加入队列时，相同的函数不会重复加入。
...
开始时间有细微不同，但是他们重置的时间是一致的，似乎是没有分两个变量的必要，所以我暂时把这理解成可以让代码更自文档吧。

说回队列，这个系统存在着**两条队列**。既然上篇说到 queueWatcher 会将需要更新的 watcher 放入队列，下次一并更新。现在就先讲 watcher 的队列，具体分析 queueWatcher 到底进行了怎样的操作。
...
最后的 `nextTick(flushSchedulerQueue)` 就是把运行 **watcher 队列**放入 **nextTick 队列中**。短短一句里连接了两个重点：`flushSchedulerQueue` 是处理 watcher 队列的核心，而 `nextTick` 是 Vue 异步渲染的核心。
...
在 `queue` 循环中，仍然会接受 `queueWatcher` push 的 watcher，这主要用于组件间触发的更新，例如父组件触发子组件更新，就不安排到下一个循环了，直接插入 `queue` 按顺序更新。
...
- setImmediate
- MessageChannel，IE 10 或以上均可使用，不查不知道，MessageChannel 的可用范围居然比 Promise 还大
- Promise，注释写用于 Weex
...
- setter 触发 watcher 后，回调函数不会立即运行，而是通过 `queueWatcher` 函数把 watcher加入队列，队列存于变量 `queue`
- 在空闲时，`queueWatcher` 函数运行 `nextTick(flushSchedulerQueue)`，把 `queue` 放到下一个 tick 运行
- `flushSchedulerQueue` 是运行 watcher 队列的函数
- `nextTick` 就跟平常用的 `$nextTick` 一样，就是把队列运行放到下一 tick
- Vue 制造下一 tick 的方法有 4 种：setImmediate、MessageChannel、Promise、setTimeout
