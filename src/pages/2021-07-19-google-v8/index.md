...
<div>
    原文链接：<a target="_blank" href="https://king-hcj.github.io/2020/10/05/google-v8/">浏览器是如何工作的：Chrome V8 让你更懂JavaScript</a><br>
    作者：<a target="_blank" href="https://github.com/king-hcj">獨釣寒江雪</a>
</div>
</div>
...
- [Ignition](https://v8.dev/docs/ignition)：interpreter，即解释器，负责将 AST 转换为 Bytecode，解释执行 Bytecode；同时收集 TurboFan 优化编译所需的信息，比如函数参数的类型；解释器执行时主要有四个模块，内存中的字节码、寄存器、栈、堆。
  ...
  > **基于寄存器的解释器架构**：![基于寄存器的解释器架构](register.jpg)
...
![V8流程](ignition-turbofan-pipeline.jpeg)
...
![bytecode-ignition](bytecode-ignition.jpg)
...
let point3 =
  (({ x: 100 } % DebugPrint(point1)) % DebugPrint(point2)) % DebugPrint(point3)
```
